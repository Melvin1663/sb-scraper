const get = require('node-fetch2');
const data = require('./data.json');
const { resolve } = require('path');
const save = require('./save.js');

get(`https://www.googleapis.com/youtube/v3/channels?part=snippet&fields=items%2Fsnippet%2Fthumbnails%2Fdefault,items%2Fsnippet%2Ftitle&id=${data.channels.join(',')}&key={key}`).then(async res => {
  if (!res || res.status >= 400) return console.log('error', res.status);
  res = await res.json();
  res.items.forEach(c => {
    let name = c.snippet.title;
    let url = c.snippet.thumbnails.default.url;
    url = url.replace(url.match(/=s\d+/gm)[0], '=');
    console.log(url);
    save(url, resolve(__dirname, `./result/${name.slice(0, 150)}.jpg`));
    console.log(name);
  })
})